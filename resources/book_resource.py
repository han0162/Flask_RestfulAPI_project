...
class DeleteResponseSchema(Schema):
    message = fields.Str(dump_only=True, description="Success message")


# 帶有id的書籍資源：用於獲取、修改、刪除書籍資訊
class BookResource(MethodResource, Resource):
    ...
    @doc(description='Delete book information', tags=['Book Request'])
    @marshal_with(DeleteResponseSchema, code=200)
    @use_kwargs(TokenSchema, location="headers")  # 從 headers 獲取 token 的 Schema
    @token_required()
    def delete(self, book_id: int, **kwargs):
        try:
            BookService().delete_book(book_id)
            return {'message': f'Book with id {book_id} has been deleted successfully'}, 200
        except Exception as error:
